<template>
	<view>
		<view class="uni-common-mt" v-show="false">
			<form>
				<view class="uni-list">
...
<script>
	...
	export default {
		...
		onLoad(options) {
			this.imageList = this.imageList.concat(JSON.parse(this.options.imageList));
		...
		onUnload() {
			...
			this.countIndex = this.count.length - 1;
		},
		methods: {
				...
				uni.chooseImage({
					...
					success: (res) => {
						...
						if (this.from === 'enter') {
							...
							this.$emit('chooseImageSuccess', params)
							uni.navigateTo({
								url: '/pages/tabBar/serviceCloud/complaint/index' + '?title='+'全民治理&imageList='+JSON.stringify(res.tempFilePaths)+'&count='+this.count.length
							});
						} else {
							const params = {
								imageList : this.imageList,
