		...
		<swiper :current="elCurrent" @change="change" @animationfinish="animationfinish" :interval="interval" :circular="circular" :duration="duration" :autoplay="autoplay"
		 ...
		 :style="{
			...
			<swiper-item class="u-swiper-item" v-for="(item, index) in list" :key="index">
				<view class="u-list-image-wrap" @click.stop.prevent="listClick(item.id, item, index)" :class="[uCurrent != index ? 'u-list-scale' : '']" :style="{
						borderRadius: `${borderRadius}rpx`,
						transform: effect3d && uCurrent != index ? 'scaleY(0.9)' : 'scaleY(1)',
						margin: effect3d && uCurrent != index ? '0 20rpx' : 0,
					}">
					<view class="status">{{item.status}}</view>
					<image class="u-swiper-image" :src="item.img" :mode="imgMode"></image>
					<view class="bottom-floor">
						<view class="title">
							{{item.title}}
						</view>
						<view class="bottom">
							<view class="dot-floor" v-if="item.from!==null">
								<text class="from">{{ item.from }}·</text>
								<!-- <view class="dot"></view> -->
								<text class="time">{{item.time}}</text>
							</view>
							<view class="info desc">评论{{item.comment}}</view>
						</view>
					</view>
				</view>
			</swiper-item>
...
<style lang="less" scoped>
	...
	/deep/uni-swiper-item {
		...
		border-radius: 20rpx;
		.u-list-image-wrap {
			box-shadow: 0px 1px 2px rgba(153,153,153,0.2);
			border-radius: 20rpx !important;
			height: 404rpx !important;
			width:460rpx !important;
		}
		.u-swiper-image {
			height: 280rpx;
